  ...
  I: I2c<SevenBitAddress, Error = E>,
{
  pub fn get_gyro_conf(&mut self) -> Result<GyroConfig, Error<E>> {
    ...
    self
      .read_bytes(defs::BMI160_GYRO_CONF_ADDR, &mut data)
      .map_err(Error::I2c)?;

    // Extract Gyro Output Data Rate (ODR) from data[0]
    let odr =
      GyroODR::try_from(data[0] & defs::BMI160_GYRO_ODR_MASK).map_err(|_| Error::InvalidMode)?;
  ...
  pub fn set_gyro_conf(&mut self, conf: GyroConfig) -> Result<(), Error<E>> {
    ...
    data[0] = (data[0] & !defs::BMI160_GYRO_BW_MASK) | ((bw_val << 4) & defs::BMI160_GYRO_BW_MASK);

    // Write updated ODR and BW (data[0]) to GYRO_CONF_ADDR
    self
      .write_u8(defs::BMI160_GYRO_CONF_ADDR, data[0])
      .map_err(Error::I2c)?;
    ...
    self
      .write_u8(defs::BMI160_GYRO_RANGE_ADDR, data[1])
      .map_err(Error::I2c)?;
  ...
  pub fn get_gyro_data(&mut self) -> Result<Vector3d<i16>, Error<E>> {
    ...
    self
      .read_bytes(defs::BMI160_GYRO_DATA_ADDR, &mut data_array)
      .map_err(Error::I2c)?;
