  ...
  I: I2c<SevenBitAddress, Error = E>,
{
  pub fn get_accel_conf(&mut self) -> Result<AccelConfig, Error<E>> {
    ...
    self
      .read_bytes(defs::BMI160_ACCEL_CONF_ADDR, &mut data)
      .map_err(Error::I2c)?;
    ...
    Ok(AccelConfig { odr, range, bandwidth: bw, undersample })
  }
  ...
  pub fn set_accel_conf(&mut self, conf: AccelConfig) -> Result<(), Error<E>> {
    ...
    data[0] = (data[0] & !defs::BMI160_ACCEL_ODR_MASK) | (odr_val & defs::BMI160_ACCEL_ODR_MASK);
    ...
    self
      .write_u8(defs::BMI160_ACCEL_CONF_ADDR, data[0])
      .map_err(Error::I2c)?;

    // --- Set Accelerometer Range ---
    data[1] =
      (data[1] & !defs::BMI160_ACCEL_RANGE_MASK) | (range_val & defs::BMI160_ACCEL_RANGE_MASK);

    // Write updated range (data[1]) to ACCEL_RANGE_ADDR
    self
      .write_u8(defs::BMI160_ACCEL_RANGE_ADDR, data[1])
      .map_err(Error::I2c)?;
  ...
  pub fn get_raw_accel_data(&mut self) -> Result<Vector3d<i16>, Error<E>> {
    ...
    self
      .read_bytes(defs::BMI160_ACCEL_DATA_ADDR, &mut data_array)
      .map_err(Error::I2c)?;
