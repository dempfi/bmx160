...
{
    pub fn set_any_motion_interrupt_conf(
        &mut self,
        conf: AnyMotionIntConf,
    ) -> Result<(), Error<E>> {
        let mut data: [u8; 2] = [0; 2];

        // Read the current INT_TAP register values
        self.read_bytes(defs::BMI160_INT_MOTION_ADDR, &mut data)
            .map_err(Error::I2c)?;
    ...
    pub fn set_significant_motion_interrupt(
        ...
        self.write_u8(defs::BMI160_INT_MOTION_ADDR + 0x3, data)
            .map_err(Error::I2c)?;
