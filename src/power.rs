    ...
    I: I2c<SevenBitAddress, Error = E>,
{
    pub fn get_power_mode(&mut self) -> Result<PowerMode, Error<E>> {
        ...
        self.read_bytes(defs::BMI160_PMU_STATUS_ADDR, &mut pmu_status)
            .map_err(Error::I2c)?;
    ...
    pub fn set_power_mode(
        ...
        if mode.accel != current.accel {
            ...
            self.write_u8(defs::BMI160_COMMAND_REG_ADDR, accel_cmd)
                .map_err(Error::I2c)?;
        ...
        if mode.gyro != current.gyro {
            self.write_u8(defs::BMI160_COMMAND_REG_ADDR, gyro_cmd)
                .map_err(Error::I2c)?;
        ...
        if mode.mag != current.mag {
            self.write_u8(defs::BMI160_COMMAND_REG_ADDR, mag_cmd)
                .map_err(Error::I2c)?;
